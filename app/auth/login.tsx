import { Keyboard, TextInput, TouchableOpacity, TouchableWithoutFeedback, View, Platform, ActivityIndicator, Modal, BackHandler } from 'react-native'
...
const Login = () => {
    ...
    const handleSubmitLogin = async () => {
        if (!isFormValid) {
            setApiError('กรุณากรอกข้อมูลให้ครบถ้วนและถูกต้อง');
            return;
        }
        setApiError('');
        ...
        try {
            ...
            setTimeout(async () => {
                if (response.data.success) {
                    ...
                    const responseAuth = await api.get('/api/users/me');
                    if (backToPage) {
                        router.navigate(backToPage as any);
                        roleNavigation(responseAuth.data.role)
                    } else {
                        roleNavigation(responseAuth.data.role)
                    }
            ...
            <LinearGradient
                ...
                <TouchableWithoutFeedback onPress={Platform.OS !== "web" ? Keyboard.dismiss : Keyboard.isVisible} accessible={false}>
                    <View style={tw`flex-1 bg-white rounded-t-[5] p-5`}>
                        ...
                        {apiError ? (
                            <View style={tw`mt-2`}>
                                <TextTheme style={tw`text-red-500 text-sm text-center`}>{apiError}</TextTheme>
                            </View>
                        ) : null}
