...
import { createReceiptDeepLink, createReceiptWebLink } from '@/app';
...
const PaymentSuccessScreen: React.FC = () => {
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      const deepLink = createReceiptDeepLink(booking.id);
      const webLink = createReceiptWebLink(booking.id);
  ...
  return (
    <ScrollView style={tw`flex-1 bg-white`}>
      ...
      <View style={tw`px-6`}>
        <ActionButtons
          onShare={handleShare}
          onBack={() => router.replace('/recent-activites')}
          isSharing={loading}
        />
      </View>
    </ScrollView>
