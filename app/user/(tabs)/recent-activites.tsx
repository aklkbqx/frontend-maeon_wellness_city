...
const BookingStatusListItem: React.FC<{
  ...
  const navigateToMap = () => {
    router.navigate({ pathname: "/user/map", params: { bookingId: booking.id } });
  };

  const navigateToPayment = () => {
    router.navigate({ pathname: "/user/payments", params: { bookingId: booking.id } });
  };
      ...
      <View style={[
        ...
        <View style={tw`p-4`}>
          <View style={tw`flex-row items-center justify-between mb-2`}>
            ...
            {!isTravelDate() && (
              <TouchableOpacity
                style={tw`flex-row justify-center items-center bg-gray-50 w-10 h-10 rounded-full`}
...
const Activity: React.FC = () => {
  ...
  if (!isLogin) {
    return (
      <View style={tw`flex-1 justify-center items-center bg-gray-50 mb-10`}>
        <View style={tw`flex-col gap-2`}>
          ...
          <View style={tw`flex-row items-center gap-2`}>
            ...
            <TouchableOpacity
              onPress={() => {
                router.navigate({
                  pathname: "/auth/login",
                  params: { backToPage: "/user/recent-activites" }
                });
              }}
            >
              <TextTheme
